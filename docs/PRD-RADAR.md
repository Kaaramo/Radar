...
> **Version** : 2.3 (Section 9 réalignée OpenClaw)
...
| v2.3    | Mai 2026  | Bootstrap monorepo + section 9 réalignée | Repo passé en monorepo turborepo : `apps/web` (Next.js 16, React 19, Tailwind 4), `packages/database` (Prisma 6, schéma 9 entités), `packages/contracts` (Zod schemas typés des 8 callbacks `/api/internal/*`). Suppression définitive de toutes les références à Fastify (notamment section 9 où traînait encore "Fastify pur" et un endpoint custom `/v1/run` côté agent). Section 9.5 entièrement réécrite : l'agent n'expose plus d'API custom, il consomme l'API OpenAI-compatible de OpenClaw (`/v1/chat/completions` sur `openclaw:18789`). Section 9.6 alignée sur le pattern callback `/api/internal/*` (cf. `KARAMO.md` + `BACHIROU.md`), avec auth par shared secret en réseau Docker interne. |
...
10. _Exigences UI/UX (Intel Dark) · à venir bloc 3_
11. _Authentification & autorisations · à venir bloc 3_
12. _Règles métier · à venir bloc 3_
13. _Contraintes techniques · à venir bloc 3_
14. _Stack technique (avec alternatives rejetées) · à venir bloc 4_
15. _Estimation des coûts · à venir bloc 4_
16. _Phasage de l'implémentation · à venir bloc 4_
17. _Indicateurs de succès · à venir bloc 4_
18. _Roadmap V2+ · à venir bloc 4_
19. _Checklist pré-implémentation · à venir bloc 5_
...
> _Citation représentative :_ « On a appris les concepts de veille en cours : cycle, SWOT, PESTEL, signaux faibles. Mais en pratique, faire tout ça manuellement pour un seul concurrent prend une journée entière. Si un outil le faisait automatiquement, on pourrait se concentrer sur l'interprétation et les recommandations. »
...
> _Citation représentative :_ « Je sais que mes concurrents bougent : ils changent leurs prix, recrutent, lancent des produits. Mais je le découvre toujours trop tard, par hasard, en scrollant LinkedIn. J'ai besoin d'un radar qui me dit chaque matin ce qui a changé chez eux. »
...
> _Citation représentative :_ « Pour chaque mission, je passe les deux premières semaines à comprendre l'écosystème concurrentiel du client. C'est du temps non facturé qui pèse sur ma marge. Si un outil me livre cette base en 48h, je peux soit gagner du temps, soit prendre plus de missions. »
...
**OpenClaw → Next.js** : Chaque sous-agent POST directement sur les routes internes `http://web:3000/api/internal/*`. Réseau interne Docker + header `X-Internal-Secret: <token>` partagé via env (`RADAR_INTERNAL_SECRET`). Pas de HMAC ni de signature : la confidentialité repose sur l'isolation réseau Docker, le secret est une ceinture-bretelles pour le cas `next dev` exposé sur localhost.
...
| POST /api/internal/\* + shared secret | Webhooks HMAC signés      | Réseau Docker interne = pas besoin de signature cryptographique. Le shared secret (`X-Internal-Secret`) suffit pour bloquer les requêtes hors-réseau si `next dev` est exposé localement. Karamo écrit en base via Prisma directement, pas de couche intermédiaire |
...
| Scheduling natif OpenClaw                       | Queue externe (pg-boss, BullMQ + Redis) | OpenClaw gère cron, persistance des sessions et reprises sur erreur en natif. Ajouter pg-boss ou Redis en V1 = 1 service de plus à opérer pour zéro gain. Migration possible en V2 si volumétrie l'exige.                                                     |
| 8 sous-agents discrets                          | 1 méga-prompt avec function calling     | Traçabilité : chaque sous-agent est testable et évaluable isolément (critique pour la soutenance M244). Coût : un méga-prompt aurait un contexte explosif et un debugging cauchemardesque.                                                                    |
| HTTP + JSON + Zod (callbacks `/api/internal/*`) | gRPC ou tRPC entre web et agent         | HTTP + JSON est universel, debuggable avec curl, pas de génération de stubs. Zod (`@radar/contracts`) donne le typage end-to-end côté TypeScript sans coupler les déploiements. tRPC envisageable plus tard si on partage davantage de logique cross-service. |
| Postgres comme stockage du métier exclusivement | Stockage métier + queue Redis séparée   | OpenClaw stocke ses sessions en interne (pas de queue externe). Postgres reste mono-source pour le métier (Rapport, Source, Swot, etc.). Simplification ops majeure en V1.                                                                                    |
...
- **Style** : REST sur HTTP/1.1 (HTTP/2 via reverse proxy en V2). Pas de tRPC en V1 pour préserver la simplicité de debug et garder une interopérabilité maximale avec OpenClaw (qui parle l'API OpenAI-compatible standard).
- **Préfixes** :
  - `/api/v1/...` : API publique consommée par le navigateur (auth, dashboard, settings).
  - `/api/internal/...` : callbacks réservés aux sous-agents OpenClaw, jamais exposés au navigateur.
  - `/v1/chat/completions` : endpoint OpenClaw consommé par Next.js (côté `openclaw:18789`, pas exposé sur Internet).
- **Format** : JSON (`Content-Type: application/json`). UTF-8 strict.
- **Validation** : Zod côté serveur. Les schémas vivent dans le package `@radar/contracts` et sont importés à la fois par `apps/web` (validation des callbacks reçus) et par `apps/agent` (typage des payloads sortants).
- **Authentification utilisateur** : cookie de session HttpOnly + SameSite=Lax (Better Auth). Pas de JWT côté client.
- **Authentification machine-to-machine** (OpenClaw ↔ Next.js) : réseau privé Docker exclusivement. Les services `web` et `openclaw` se parlent via leur nom DNS interne. **En complément** : header `X-Internal-Secret: <token>` partagé via env (`RADAR_INTERNAL_SECRET`), validé par middleware Next.js sur tout `/api/internal/*`. Cette ceinture-bretelles protège le cas où `next dev` est lancé localement (port 3000 exposé sur localhost). Pas de HMAC ni de signature : la confidentialité repose sur l'isolation réseau et le secret partagé.
...
### 9.5 Trigger Next.js → OpenClaw

OpenClaw n'expose **pas d'API custom**. Next.js déclenche les cycles via l'API OpenAI-compatible standard du service `openclaw:18789`, accessible uniquement sur le réseau privé Docker.

| Méthode | Endpoint                                    | Body                                                                                        | Réponse                                                             |
| ------- | ------------------------------------------- | ------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| POST    | `http://openclaw:18789/v1/chat/completions` | `{ model: "openclaw/default", messages: [{ role: "user", content: <payload structuré> }] }` | 202 (OpenClaw exécute le pipeline en arrière-plan, retour immédiat) |

**Format du `content`** : à figer entre Karamo et Bachirou (cf. `BACHIROU.md` § 4.1). Le langage naturel n'est pas type-safe, donc le format retenu est un bloc clé-valeur strict ou un tool call OpenClaw, pas une phrase libre. Exemples de cas d'usage :

| Cas d'usage                | Quand                     | Payload                                                       |
| -------------------------- | ------------------------- | ------------------------------------------------------------- |
| `deep-research.user`       | Fin onboarding (1 fois)   | `userId`, `nomEntreprise`, `siteWeb`                          |
| `deep-research.competitor` | Ajout d'un concurrent     | `userId`, `competitorId`, `nom`, `siteWeb`                    |
| `cycle.daily`              | Cron 6h ou trigger manuel | `userId`, `rapportId`, `concurrentId`, `profil` (JSON inline) |
| `cycle.weekly_pestel`      | Cron lundi 6h             | `userId`, `secteur`, `rapportId`                              |
| `export.pdf`               | Demande utilisateur       | `userId`, `exportId`, `type`, filtres                         |

**Idempotence et retry** (à arbitrer côté SKILL.md de l'orchestrateur, cf. `BACHIROU.md` § 4.3) : 3 retries avec backoff exponentiel par sous-agent, payloads idempotents côté Next.js (UPSERT par `rapportId` sauf pour `Source` et `SignalFaible` qui sont append-only), bascule automatique vers `POST /api/internal/rapport/echec` après abandon.

### 9.6 Callbacks OpenClaw → Next.js (`/api/internal/*`)

Chaque sous-agent OpenClaw POST ses résultats sur l'une des 8 routes internes de `apps/web`. Réseau Docker interne + header `X-Internal-Secret` (cf. § 9.1). Les contrats Zod sont dans `@radar/contracts/internal`.

| Méthode | Endpoint                          | Émetteur                       | Schéma Zod                    | Sémantique                                                                                    |
| ------- | --------------------------------- | ------------------------------ | ----------------------------- | --------------------------------------------------------------------------------------------- |
| POST    | `/api/internal/profil`            | `deep-research`                | `ProfilEventSchema`           | UPSERT du `ProfilUtilisateur` à la fin de l'onboarding                                        |
| POST    | `/api/internal/rapport/progresse` | tout sous-agent                | `RapportProgresseEventSchema` | UPDATE `Rapport.statut`, `progressionPct`, `etape`                                            |
| POST    | `/api/internal/sources`           | `collecteur` puis `evaluateur` | `SourcesEventSchema`          | INSERT (append-only) `Source[]` avec score CRAAP                                              |
| POST    | `/api/internal/swot`              | `analyste-swot`                | `SwotEventSchema`             | UPSERT `Swot` (1 par `Rapport`)                                                               |
| POST    | `/api/internal/pestel`            | `analyste-pestel`              | `PestelEventSchema`           | UPSERT `Pestel` (1 par `Rapport`)                                                             |
| POST    | `/api/internal/signaux`           | `detecteur-signaux-faibles`    | `SignauxEventSchema`          | INSERT (append-only) `SignalFaible[]`                                                         |
| POST    | `/api/internal/rapport/termine`   | `redacteur`                    | `RapportTermineEventSchema`   | UPDATE `Rapport.statut=TERMINE`, `synthese`, `termineLe`                                      |
| POST    | `/api/internal/rapport/echec`     | `orchestrateur`                | `RapportEchecEventSchema`     | UPDATE `Rapport.statut=ECHEC`, `erreur`. Émis automatiquement après 3 retries d'un sous-agent |

Toutes les réponses sont 200/204 en succès, 401 si secret invalide, 422 si payload Zod invalide, 500 sur erreur serveur (déclenche retry côté OpenClaw).

### 9.7 Webhooks entrants externes

Endpoints publics réservés à des fournisseurs tiers (Resend en V1). Authentification spécifique au fournisseur, pas le secret interne.

| Méthode | Endpoint               | Source | Body                                              |
| ------- | ---------------------- | ------ | ------------------------------------------------- |
| POST    | `/api/webhooks/resend` | Resend | Format Resend (signé via `RESEND_WEBHOOK_SECRET`) |

### 9.8 Documentation OpenAPI
