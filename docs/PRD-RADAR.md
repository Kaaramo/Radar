...
| v2.3 | Mai 2026 | Migration LLM + provider web search | LLM migré de Claude Opus 4.7 vers DeepSeek V4 Pro (`deepseek/deepseek-v4-pro`, thinking=medium, contexte 1M tokens). Tavily réintégré comme provider web_search primaire (TAVILY_API_KEY) — DuckDuckGo conservé en fallback manuel via logique conditionnelle dans collecteur/SKILL.md. Cron 6h00 géré par Next.js (node-cron) et non par OpenClaw natif : Next.js interroge la base, récupère les profils actifs, et déclenche un orchestrateur par utilisateur avec le profil complet dans le message. timeoutSeconds global porté à 900s. 8 SKILL.md validés par tests d'intégration complets (rapport-test-005, Mai 2026). |
