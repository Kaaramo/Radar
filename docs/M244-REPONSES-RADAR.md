# M244 : Réponses RADAR aux consignes du Pr. Wadiai

> **Document de travail pour la préparation de la soutenance et du rapport final.**
>
> Ce fichier répond, section par section, à toutes les questions et exigences soulevées par le Pr. Wadiai dans `M244 Consignes Techniques de veille.pdf`. Les consignes brutes sont reproduites fidèlement dans `docs/M244-CONSIGNES-PROFESSEUR.md`.
>
> **Sources croisées** : PRD-RADAR v2.3, Charte Graphique RADAR Editorial, KARAMO.md, BACHIROU.md, CLAUDE.md, specs Sprints 1 à 3.
> **Projet** : RADAR, outil de veille concurrentielle propulsé par un agent IA autonome (OpenClaw).
> **Slogan officiel** : « Vos concurrents bougent. Radar vous le dit avant tout le monde. »
...
| #   | Section répondue                                            | Renvoi consignes |
| --- | ----------------------------------------------------------- | ---------------- |
| 01  | Identité institutionnelle et identité du projet             | PDF p. 1         |
| 02  | Comment RADAR répond à l'objectif du guide                  | PDF p. 2         |
| 03  | Présentation du sujet (thème, contexte, importance, impact) | PDF p. 3         |
| 04  | Problématique (formulation, besoin, difficultés, enjeux)    | PDF p. 4         |
| 05  | Objectifs du projet (principal, secondaires, résultats)     | PDF p. 5         |
| 06  | Veille et recherche documentaire                            | PDF p. 6         |
| 07  | Méthodologie de recherche                                   | PDF p. 7         |
| 08  | Planification du projet                                     | PDF p. 8         |
| 09  | Présentation de la solution                                 | PDF p. 9         |
| 10  | Réalisation technique                                       | PDF p. 10        |
| 11  | Démonstration du projet                                     | PDF p. 11        |
| 12  | Tests et validation                                         | PDF p. 12        |
| 13  | Analyse critique                                            | PDF p. 13        |
| 14  | Recommandations                                             | PDF p. 14        |
| 15  | Dimension professionnelle : le cahier des charges           | PDF p. 15        |
| 16  | Dimension professionnelle : le rapport final                | PDF p. 16        |
| 17  | Consignes importantes pour le rapport                       | PDF p. 17        |
| 18  | Bibliographie et ressources                                 | PDF p. 18        |
| 19  | Conseils présentation orale : règles et temps               | PDF p. 19        |
| 20  | Conseils présentation orale : apparence professionnelle     | PDF p. 20        |

---

## 01 · Identité institutionnelle et identité du projet
...
## 02 · Comment RADAR répond à l'objectif du guide

Le projet RADAR a été conçu dès l'origine pour répondre aux quatre objectifs énoncés par le Pr. Wadiai (présentation pro, analyse critique, structure claire, défense efficace) :
...
4. **Défense efficace** : la démonstration vivante d'un cycle de veille de bout en bout (cron 06h00, collecte, CRAAP, SWOT, PESTEL, signaux faibles, rédaction, digest email) sera exécutée en direct devant le jury. Cette preuve par l'exécution vaut plus que toute description écrite.
...
RADAR n'est pas un projet de génie logiciel auquel on aurait ajouté a posteriori une couche de vocabulaire de veille. La méthodologie enseignée dans le module M244 par le Pr. Wadiai est le **moteur du produit** : le cycle de veille en 5 étapes (Chapitre 1), la grille d'évaluation CRAAP (Chapitre 3), les grilles SWOT et PESTEL (Chapitre 2), la détection de signaux faibles (Chapitre 1) et la diffusion structurée (Chapitre 3) sont codés dans les prompts de l'agent et structurent chaque rapport généré.
...
5. **Importance professionnelle** : le projet constitue une pièce de portfolio défendable face aux recruteurs en stratégie, data ou IA appliquée, en particulier pour Karamo Sylla et Bachirou Konaté qui sortent du Cycle Ingénieur BDIA.
...
Le projet est volontairement **agnostique du secteur d'activité** des entreprises surveillées : le moteur de veille ne dépend pas du secteur, seuls les concurrents et axes de surveillance changent.
...
Le problème opérationnel observé chez les utilisateurs cibles (dirigeants PME, consultants indépendants, étudiants en stratégie) recouvre cinq défaillances structurelles cumulatives :
...
- North Star Metric V1 : 10 mouvements concurrents validés (deux sources minimum) par utilisateur actif et par semaine
...
Trois besoins utilisateurs ont été identifiés à partir des trois personas adressables :

| Persona                | Besoin formulé                                                                                                                                            |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Étudiant Stratégiste   | Démontrer la maîtrise des concepts M244 dans un projet livrable, gagner du temps sur la collecte pour se concentrer sur l'interprétation                  |
| Dirigeant PME          | Être alerté dès qu'un concurrent bouge, avoir une vue synthétique sans se perdre dans le détail, prendre des décisions informées sur des données fraîches |
| Consultant indépendant | Accélérer la phase d'audit concurrentiel facturée (20 à 30 % du temps non facturable), maintenir une veille continue sur 3 ou 4 comptes en retainer       |
...
1. **Difficulté méthodologique** : la grille CRAAP du Chapitre 3 du cours est définie pour une évaluation humaine. La transformer en algorithme calculable par un LLM nécessite de coder chaque dimension (Currency, Relevance, Authority, Accuracy, Purpose) en critères opérables sur un texte brut récupéré du web. Solution : prompt structuré dans `evaluateur/SKILL.md`.
2. **Difficulté technique d'orchestration** : un cycle de veille complet enchaîne 5 sous-agents séquentiels et nécessite environ 30 minutes par concurrent. Cette durée est incompatible avec le timeout serverless typique (10 s à 5 min). Solution : service Node dédié (`apps/agent` initialement, remplacé par OpenClaw en architecture finale), cycle asynchrone, callbacks `/api/internal/*`.
...
5. **Difficulté de gouvernance des contrats inter-services** : Next.js et OpenClaw communiquent par 8 endpoints `/api/internal/*`. Sans typage partagé, un champ ajouté côté agent et inconnu côté web devient un bug silencieux. Solution : `packages/contracts` avec 8 schemas Zod typés, importés des deux côtés.
...
| Enjeu de pérennité | Documenter le projet de bout en bout pour qu'il survive à la soutenance                                                                |

### 4.5 Lien avec le monde professionnel

Le lien avec le monde professionnel est porté par trois preuves concrètes :

1. **Architecture orientée production**, pas démonstrateur jetable : `docker-compose.yml` à 3 services, conformité RGPD (soft-delete + purge à T+7), audit log sur toutes les mutations utilisateur, validation Zod end-to-end, observabilité native, rate-limiting Postgres-backed.
2. **Personas utilisateurs réels et adressables** : Dirigeants de PME marocaines, consultants indépendants à Casablanca, Rabat, Tunis, Dakar, étudiants en Master 2 stratégie. Le projet ne décrit pas des personae fictives mais des segments économiques observés.
...
L'objectif principal est mesurable par trois critères de réussite cumulatifs :

1. Le MVP V1 livre les 7 fonctionnalités obligatoires (F1 à F7, MoSCoW Must, sauf F6 en Should) qui couvrent l'intégralité du parcours utilisateur.
...
| 7   | Répondre  | Répondre au besoin de défense académique par une documentation exhaustive                                                              |
...
- 1 schéma Prisma à 20 entités (V2 cible), version simplifiée 9 entités en V1
...
- 1 proposition de valeur formulée (« Vos concurrents bougent. Radar vous le dit avant tout le monde. ») et défendue dans le PRD
- 3 personas adressables documentés avec citations représentatives et Jobs to Be Done
- 1 anti-vision claire qui protège la cohérence produit en V1
- 1 North Star Metric défini avec cibles V1 et V2
...
Pour la phase de benchmark concurrentiel :

- `similarweb.com`, `semrush.com`, `crayon.co`, `klue.com`, `kompyte.com`, `visualping.io`, `mention.com`, `brandwatch.com`, `feedly.com`
- Pour le contexte marché Maroc et Maghreb : `hcp.ma` (Haut-Commissariat au Plan), `cgem.ma` (Confédération Générale des Entreprises du Maroc), `medias24.com`, `lavieeco.com`
...
| Documentation RGPD CNIL et CNDP (Maroc)                                        | Conformité du soft-delete, du droit à l'oubli, du droit d'accès                      |
| Cours M244 du Pr. Wadiai (chapitres 1, 2, 3)                                   | Source de vérité méthodologique encodée dans l'agent                                 |
...
| pgvector (Postgres extension)              | Déduplication sémantique des `Movement` détectés                                           |
...
1. **Analyse comparative** : benchmark des outils existants (Similarweb, SEMrush, Crayon, Klue, Visualping, Mention, Brandwatch, Feedly) pour identifier les vides du marché et positionner RADAR sur un segment distinct.
2. **Étude qualitative** : construction de 3 personas adressables (Étudiant Stratégiste, Dirigeant PME, Consultant indépendant) avec citations représentatives et Jobs to Be Done, plus 5 anti-personas pour protéger la cohérence produit.
3. **Étude quantitative** : volumétrie des données projetée, estimation des coûts d'infrastructure (4 EUR/mois VPS Hetzner CX21), estimation des coûts LLM Claude Opus 4.7.
4. **Tests techniques** : prototypage incrémental sur 6 sprints, validation de la chaîne complète (Postgres + OpenClaw + Next.js) en environnement Docker local, vérification `pnpm exec turbo run type-check` et `pnpm exec turbo run build` à chaque commit.
...
| 2   | Étude de marché et analyse concurrentielle | Table comparative des outils, anti-personas                          | Livré    |
| 3   | Conception de l'architecture technique     | Engine OpenClaw, 8 sous-agents, callbacks `/api/internal/*`          | Livré    |
| 4   | Conception du modèle de données            | Prisma 9 entités V1, 20 entités V2, seed et migration strategy       | Livré    |
...
Le projet est conduit en binôme par **Karamo Sylla** et **Bachirou Konaté**, étudiants du Cycle Ingénieur BDIA à l'ENSA Tétouan. L'organisation est strictement **modulaire et asynchrone** : chaque membre a un périmètre de responsabilité défini, qui correspond à un sous-arbre du monorepo.
...
Cette modularité s'appuie sur les contrats Zod partagés (`@radar/contracts`) qui garantissent que les deux périmètres se parlent sans casser : un schema Zod est la source de vérité, importé à la fois par `apps/web` et `apps/agent`.
...
Diagramme de Gantt visuel à produire pour la soutenance (recommandation du professeur). Outil envisagé : Mermaid intégré au rapport, ou export Figma.
...
La gestion du temps suit cinq principes opérationnels :
...
Le cycle de fonctionnement nominal enchaîne quatre phases :
...
Le **moment AHA** est précisément le jour 2 : l'utilisateur reçoit son second digest et constate que les alertes enrichissent celles de la veille (déduplication fonctionne), que la SWOT a évolué (continuité analytique), et qu'un signal faible apparaît, croisé sur deux jours, qu'il n'aurait jamais repéré seul.

### 9.3 Architecture

L'architecture repose sur trois conteneurs Docker collaborant sur un réseau interne :
...
La valeur ajoutée de RADAR se mesure sur cinq axes :
...
Le développement couvre 7 fonctionnalités (F1 à F7), réparties sur 6 sprints :
...
Recommandation Pr. Wadiai : utiliser des captures d'écran pour illustrer les fonctionnalités. Liste minimale à préparer (1 capture par item) :
...
La démonstration en direct est l'épine dorsale de la défense. Conformément au principe de la « démonstration vivante », un MVP fonctionnel qui surveille réellement deux ou trois concurrents et produit un rapport en direct devant le jury vaut plus que toute description écrite.
...
**Préparation technique anti-incident** :
...
Trois cas d'usage à dérouler pour matérialiser les trois personas adressables :
...
**Validation des contrats inter-services** : tous les payloads échangés entre Next.js et OpenClaw passent par les schemas Zod de `@radar/contracts`. Un payload mal formé est rejeté côté Next.js avec 422, ce qui force OpenClaw à corriger ou à retry (3 fois avec backoff exponentiel).
...
Sur le plan applicatif, les corrections sont tracées dans `tasks/lessons.md`.
...
5. **Pas de mobile native en V1** : web responsive uniquement. App mobile native explicitement « Won't » du MoSCoW V1.
...
**Lecture quantitative** (volumétrie testée en V1) :
...
| Conformité RGPD intégrée by design             | Soft-delete + purge T+7, audit log, droit d'accès, base légale documentée.                                                          |
...
| Pas de file d'attente externe (pas de pg-boss)           | Choix assumé pour la V1, mais limite la scalabilité au-delà de quelques dizaines d'utilisateurs simultanés.                      |
...
| Conflit de structure entre 3 dossiers initiaux (`frontend/`, `backend/`, `database/`) et monorepo Turborepo | PR #1 de Bachirou mergée avec stratégie `theirs` sur 3 conflits modify/delete                    |
| Coordination binôme inter-périmètres (Karamo Next.js / Bachirou Agent)                                      | Document de spécifications croisées, 4 points de vigilance, 5 décisions à trancher               |
| Format du trigger Next.js → OpenClaw (langage naturel fragile)                                              | Bascule vers format strict ou tool calls OpenClaw                                                |
| Auth sur `/api/internal/*` initialement absente                                                             | Ajout d'un header `X-Internal-Secret` partagé via env, validé par middleware Next.js             |
| Idempotence et retry non spécifiés                                                                          | Décision : 3 retries backoff exponentiel, UPSERT côté Next.js, échec automatique après abandon   |
| Versioning des contrats inter-services                                                                      | `@radar/contracts` désigné comme source de vérité, toute modification passe en PR review croisée |
...
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Migration sur Vercel pour `apps/web`                       | Edge functions, déploiement preview par PR, scalabilité automatique                                     |
...
| Ajout d'une couche de cache sémantique sur les Movements   | pgvector + similarité cosine pour dédupliquer les Movements similaires sur 30 jours                     |
...
Les besoins identifiés sont ceux des 3 personas adressables :
...
| Conformité RGPD        | Soft-delete + purge T+7, audit log, droit d'accès, base légale documentée                    |
...
| Mobile                 | Web responsive desktop-first 1280px puis adapté 640px                                        |
...
Le rapport final suit la structure académique du Chapitre 2 du module M244, enrichie des sections imposées par le guide du Pr. Wadiai.
...
Répartition de la parole calquée sur la répartition des périmètres :
...
- Bachirou connaît `apps/web/`, le schéma Prisma et les écrans (lit le PRD et la charte).
...
| ---------------------------------------------- | -------------------------------------------------------------------------------------------------- |
| Question conceptuelle (M244)                   | Référer au cours et à l'ancrage M244 du projet                                                     |
| Question technique pointue (Anthropic, Docker) | Réponse technique précise + référence à la doc officielle                                          |
| Question business (modèle économique)          | Référer au positionnement et à la roadmap V2                                                       |
| Question critique sur une faiblesse            | Reconnaître honnêtement (§ 13.3), expliquer la mitigation envisagée                                |
| Question hors scope                            | Reconnaître le hors scope, renvoyer à la roadmap V2 ou à l'anti-vision du projet                   |
...
## Annexe A · Documents sources à exploiter pour le rapport final
...
| Reproduction Markdown des consignes | `docs/M244-CONSIGNES-PROFESSEUR.md`                        | Référence sans omission                    |

---

> **Fin du document.** Ce fichier est la source unique de réponses RADAR aux consignes du module M244. Il est conçu pour être lu en regard de `docs/M244-CONSIGNES-PROFESSEUR.md`.
